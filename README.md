...
- Edit product lists in `lib/catalog.ts`.
- Upload single audio files into `private_downloads/` and ensure `fileKey` matches.
- Configure Vercel env vars:
  - `STRIPE_SECRET_KEY`
  - `NEXT_PUBLIC_SITE_URL` (e.g., https://kp-rho-nine.vercel.app)

Pages:
- `/singles` Buy & Download
- `/merch` Merch shop


### Webhook (Email download links)

To email buyers their download link after purchase (optional):
- Add `STRIPE_WEBHOOK_SECRET` in Vercel env vars
- Create a Stripe webhook endpoint pointing to `/api/webhook/stripe`
- (Optional) Add `RESEND_API_KEY` and `RESEND_FROM` to send emails

Merch shipping is configured in `app/api/checkout/route.ts` (fixed Standard/Express rates).
